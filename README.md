# CSCE-2110-Project-1
FINAL
Your README file must include:
names of all group members

Zackary Sturtevant : Main.cpp, QueueSystem.h, QueueSystem.cpp

Brolyn Ingosi : StudentList.h, StudentList.cpp

Emilio Palacio : CampusMap.h, CampusMap.cpp

Najeeb Rafeeq : FileManager.h, FileManager.cpp, campus_map.txt,  students.txt


compilation instructions : make sure you have the two .txt files in same folder as code and type "g++ Main.cpp QueueSystem.cpp FileManager.cpp Campusmap.cpp StudentList.cpp -o program"

execution instructions
description of implemented features
known bugs or limitations

PUSH ANY CHANGES TO MAIN.CPP TO m.cpp_testing

--TRUE FINAL--
